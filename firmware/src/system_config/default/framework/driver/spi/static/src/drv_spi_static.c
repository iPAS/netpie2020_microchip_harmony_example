...
SYS_MODULE_OBJ DRV_SPI0_Initialize(void)
{
    ...
    /* Baud rate selection */
    PLIB_SPI_BaudRateSet( SPI_ID_1 , SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 8000000 );
    ...
    dObj->sendDMAHander = DRV_SPI0_ISRDMAMasterSendEventHandler8bit;
    dObj->receiveDMAHander = DRV_SPI0_ISRDMAMasterReceiveEventHandler8bit;
    dObj->operationStarting = NULL;
    dObj->operationEnded = NULL;
    dObj->txDmaThreshold = 16;
    dObj->txDmaChannelHandle = SYS_DMA_CHANNEL_HANDLE_INVALID;
    dObj->rxDmaThreshold = 16;
